...
if not mox.file.exists(args.tmp_save_dir):
    mox.file.make_dirs(args.tmp_save_dir)

source = os.path.join(args.data_url)
target = os.path.join(args.tmp_data_dir)
mox.file.copy_parallel(source, target)

tmp_resume = os.path.join(args.tmp_save_dir, 'resume.pth')
if args.resume:
    mox.file.copy_parallel(args.resume, tmp_resume)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    filename = args.tmp_save_dir+filename
